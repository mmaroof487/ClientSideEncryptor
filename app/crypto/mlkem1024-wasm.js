 ...
 * @returns {Uint8Array}
 */
export function decap(private_key, ciphertext) {
	try {
		const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
		const ptr0 = passArray8ToWasm0(private_key, wasm.__wbindgen_export2);
		const len0 = WASM_VECTOR_LEN;
		const ptr1 = passArray8ToWasm0(ciphertext, wasm.__wbindgen_export2);
		const len1 = WASM_VECTOR_LEN;
		wasm.decap(retptr, ptr0, len0, ptr1, len1);
...
async function __wbg_init(module_or_path) {
	...
	if (typeof module_or_path === "undefined") {
		throw new Error("WASM path must be provided explicitly");
	}
	const imports = __wbg_get_imports();
